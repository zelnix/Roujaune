...
## Real-time 3D rider (2026-06-23)
- **Rejected the 2D SVG avatar** for lacking realism; deleted `src/avatar/` (+ old `/avatar-demo` route). Pivoted to a real-time **3D cyclist** via `react-three-fiber` + `expo-gl` (`three`).
- `src/avatar3d/Rider3D.tsx`: loads a free **rigged Mixamo human** (male = three.js **Xbot**, female = **Michelle**, both `mixamorig:` rigs) from the stable **threejs.org** CDN (Ready Player Me's public service shut down 2026-01-31, its CDN is gone; threejs.org/githubusercontent both serve `ACAO:*` so they work in web preview + native). Renders a **procedural 3D road bike** (frame/wheels/spokes/crank/pedals/saddle/bars from primitives).
- **Procedural pedalling:** crank + wheels advance from `cadence`; legs solved with **planar 2-bone IK** each frame so both feet stay locked to the rotating pedals (180° opposed). Torso lean grows with `power` + standing; **seated↔standing** eased blend (hip lift + sway) driven by `useAutoStanding` (hysteresis on power/cadence). Arms posed to the bars from T-pose via world-space bone rotations (helper `setWorld(ax,az,ay)`, relative to captured bind world quats). Pauses freeze the crank.
- **4 ROUJAUNE kit presets** (`KIT_PRESETS`): Yellow/Burgundy + Black/Burgundy (male Xbot), Female·Black + Female·Pink (female Michelle) — jersey colour tinted live on the body material. Preset persisted via `prefs.getKitPreset/setKitPreset`; picked in `src/avatar3d/KitSheet.tsx` (opens from the workout **Rider** button).
- Standalone tuning screen `/avatar-3d` (cadence/power/standing/paused steppers + preset picker). Integrated into `/workout` as a bottom-left overlay fed by live telemetry.
  - **NOTE:** R3F/native `Canvas` measures its **container** on web — the overlay wrapper MUST have explicit width/height or the GL context never initialises (0×0). YouTube video + TTS audio still can't be validated headless; the 3D canvas DOES render in the screenshot tool.
  - **Limitation:** Michelle's single textured material means the female kit recolour tints skin too; the male mannequin recolours cleanly. Both are free rigged mannequins (not photoreal cyclists) — the realism ceiling given RPM's shutdown. A premium purchased/custom rigged GLB can be dropped in later by swapping `BODY_URL`.
