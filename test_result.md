...
test_plan:
  current_focus:
    - "P0 BUGFIX (both): 'Next Workouts' rows on the Training Plan card no longer disappear once live data loads. Root cause: backend _ctr_plan_response only emitted the CURRENT week's rides (often 0-1 incomplete), so the list collapsed. Fix: builder now emits ALL day types (cycling, recovery, rest, strength/mobility/balance) in chronological order across whole weeks until >=5 incomplete entries exist. GET /api/plan (couch-to-road, ride-stronger, ride-beyond) should return workouts[] with a persistent list including type/subtitle fields."
    - "NEW REQUIREMENT: Training Plan card shows ALL workouts incl recovery + rest days, not just rides. Each workout has type (cycling|recovery|rest|strength|mobility|balance), and non-cycling ones carry subtitle + icon + color. completed flags: cycling by ride_id, rest by past-date, recovery/supp by supp_dates."
    - "NEW BACKEND ENDPOINT: GET /api/progress/timeline?range=week|month|3m|6m|1y&offset=N (auth required). Buckets real ride_history into a scrollable timeline. Returns range, offset, has_next (offset>0), window_label, buckets[{label,tss,hours,rides}], summary{rides,hours,tss,distance_km,elevation_m,avg_power,tss_delta_pct}, recent[]. offset scrolls whole windows into the past."
    - "FRONTEND Today screen: nav label 'Home' renamed to 'Today'; Wellness card moved into the wide mid slot (in place of Calendar); Calendar moved to bottom row next to Community; standalone Achievement card removed; Achievements now shown as a compact strip inside the Progress card (testID progress-achievements)."
    - "FRONTEND Progress screen: new Training Timeline card (testID progress-timeline) with range chips (tl-range-week/month/3m/6m/1y), prev/next date scroll (tl-prev / tl-next; tl-next disabled at offset 0), a TSS bar chart, and summary stats. Empty-state when no rides in window."
    - "FRONTEND Virtual Routes: setup-screen Exit button (testID vr-exit) enlarged into a prominent bordered pill."
  stuck_tasks: []
  test_all: false
  test_priority: "high_first"
...
agent_communication:
    -agent: "main"
    -comment: "FORK CONTINUATION — implemented user msg 569 batch + a follow-up. (1) FIXED recurring 'Next Workouts disappear' bug at the real root cause (backend only returned current-week rides). Now _ctr_plan_response emits all day types across weeks until >=5 upcoming. (2) Card now shows recovery/rest/strength/etc in addition to rides. (3) Added GET /api/progress/timeline endpoint bucketing real ride_history (week/month/3m/6m/1y with offset scroll). (4) Renamed Home->Today, swapped Wellness<->Calendar positions on Today, merged Achievements into Progress card. (5) New Progress Timeline UI. (6) Enlarged Virtual Routes Exit button. Verified via screenshots: Today cards render with mixed workout types, Progress timeline shows 8 rides/460 TSS for 3M, exit pill enlarged. TEST REQUEST: BACKEND — GET /api/plan returns persistent mixed-type workouts[] (>=5 incomplete, includes non-cycling types with type/subtitle); GET /api/progress/timeline all ranges + offset (has_next logic, bucket counts: week=7, month=4, 3m=13, 6m=6, 1y=12, summary aggregation). FRONTEND — Today screen (Today label, Wellness in mid slot, Calendar bottom, Progress card achievements strip, Training Plan card mixed types persist and don't vanish), Progress timeline (range chips switch, prev/next scroll, tl-next disabled at present). Credentials greenlantern@roujaune.app / rideon9900. Do NOT ride routes to completion. Clean up any rider_prs/ride_history docs you create."
