...
test_plan:
  current_focus:
    - "Ride Beyond (Advanced default plan, Phase 1 Weeks 1-4) — end-to-end"
    - "GET /api/plans lists ride-beyond (structured, 4 weeks, level Advanced)"
    - "POST /api/onboarding/recommend for an Advanced profile returns ride-beyond with authored:true"
    - "POST /api/rider/plan {plan_id:'ride-beyond', reset_progress:true} assigns it; GET /api/plan returns rich structured response (phase 'Performance Foundation', workouts rb-ride-1..3, progress '1 / 4')"
    - "GET /api/calendar/week renders Tue rb-ride-1, Thu rb-ride-2, Sat rb-ride-3, Wed strength (FB50), Fri recovery, Mon/Sun rest"
    - "Live Workout launches rb-ride-1 (Advanced Performance Baseline) and runs the authored interval timeline"
    - "Beginner couch-to-road and Intermediate ride-stronger still work unchanged (no regression)"
    - "FRONTEND: /plan phase-completion celebration modal fires once when a phase reaches 100% (testID phase-celebration-continue)"
...
agent_communication:
    -agent: "main"
    -comment: "NEW: 'Ride Beyond' Advanced default plan (Phase 1, Weeks 1-4 = 4 weeks / 12 rides rb-ride-1..12). Authored in frontend/src/lib/programs/ride-beyond.ts, wired into Live Workout via RB_WORKOUTS in workout-catalog.ts, backend def ride_beyond_plan.json (generated by scripts/export-ride-beyond.js), seeded + force-refreshed on startup in server.py (mirrors ride-stronger pipeline incl. non-clobber fix). Advanced onboarding now recommends ride-beyond (LEVEL_PLAN Advanced=ride-beyond, tagged level Advanced). Self-verified via curl: GET /api/plans shows ride-beyond 4 weeks; switch+GET /api/plan returns Ride Beyond, phases[Performance Foundation], calendar Tue/Thu/Sat rb-ride-1/2/3. Green Lantern restored to couch-to-road. ALSO: (1) Phase-completion celebration modal (src/components/PhaseCelebrationModal.tsx + src/lib/phase-complete.ts hook usePhaseCelebration) wired into app/plan.tsx — fires once per completed phase using authored phase.complete{heading,summary,coachMessage}; visually verified via screenshot with ride-stronger phase1 at 100%. (2) Fixed negative-width SVG warning by clamping bar width/height >=0 in WeeklyLoadChart + WorkoutProfile. Test creds: greenlantern@roujaune.app / rideon9900."
