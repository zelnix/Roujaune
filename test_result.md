...
test_plan:
  current_focus:
    - "PHASE 2 — Training Plan deep-dives. BACKEND: (1) GET /api/plan phases[] now include an `objective` string per phase. (2) NEW POST /api/coach/adaptation/detail {plan_id,coach_name,coach_gender,refresh} → {detail:{summary, factors:[{label,detail}], adjustments:[...]}}, AI-generated (Claude), cached per plan+coach; for couch-to-road/ride-stronger/ride-beyond it is grounded in the COMPUTED plan (phase/progress). (3) rider_appearance is now user-scoped (added to USER_SCOPED in auth.py)."
    - "FRONTEND Training Plan screen (/plan): Phases tab now renders CurrentPhaseRoadmap + a new inline PhasesDetailCard (testID phases-detail) listing every phase with objective, progress bar and a CURRENT chip on the active phase (testID phase-row-<id>, tap opens PhaseDetailModal testID phase-detail-<id>). Tapping a phase card in the roadmap also opens PhaseDetailModal."
    - "FRONTEND Key Workouts: tapping a workout card (testID workout-<id>) now opens KeyWorkoutDetailModal (title, zone/duration/TSS, WorkoutProfile, WORKOUT STEPS from buildSegments, and an 'Open in Training' footer button testID workout-detail-open) INSTEAD of navigating away. Key Workouts grid is filtered to cycling entries (they have profile)."
    - "FRONTEND Adaptations: AdaptationsModal now shows a 'WHY <coach> ADJUSTED YOUR PLAN' reasoning block (testID adaptation-reasoning) — summary + factor list + WHAT CHANGED adjustments — fetched from /api/coach/adaptation/detail, above the existing adaptation history timeline. planId passed from the active plan."
    - "FRONTEND Weekly Load: WeeklyLoadCard now has a stats breakdown row (testID weekly-load-stats): This Week, Avg/Week, Peak (week), Completed %."
  stuck_tasks: []
  test_all: false
  test_priority: "high_first"
...
agent_communication:
    -agent: "main"
    -comment: "PHASE 2 (Training Plan deep-dives) + rider_appearance user-scoping. Screenshots already confirm: Phases tab detailed breakdown with current-phase highlight, and the Adaptations AI reasoning modal (grounded, 4 factors + adjustments) render correctly. TEST REQUEST — BACKEND: (a) GET /api/plan phases[] have `objective`; (b) POST /api/coach/adaptation/detail returns well-formed {summary, factors[], adjustments[]} for couch-to-road (grounded, non-empty), and caches (second call cached=true, refresh=true regenerates); (c) rider_appearance now user-scoped — GET/PUT /api/rider/appearance works per user. FRONTEND (landscape ~1024x720): (1) /plan Phases tab shows phases-detail with CURRENT chip on Get Moving + objectives; tapping phase-row opens the phase detail modal. (2) Key Workouts tab: tapping a workout opens KeyWorkoutDetailModal with steps + 'Open in Training'. (3) Adaptations: opening the adaptations modal shows the 'WHY ALBERTO ADJUSTED YOUR PLAN' reasoning block. (4) Weekly Load (Load & Progress tab) shows the weekly-load-stats row. Credentials greenlantern@roujaune.app / rideon9900. Do NOT ride to completion. Leave data pristine (coach_id=alberto)."
