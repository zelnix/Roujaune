...
test_plan:
  current_focus:
    - "Ride Stronger (Intermediate default plan) — end-to-end"
    - "GET /api/plans lists ride-stronger (structured, 4 weeks)"
    - "POST /api/onboarding/recommend for an Intermediate profile returns ride-stronger with authored:true"
    - "POST /api/rider/plan {plan_id:'ride-stronger', reset_progress:true} assigns it; GET /api/plan returns rich structured response (phase, workouts rs-ride-1..3, progress '1 / 4')"
    - "GET /api/calendar/week renders Mon strength / Tue,Thu,Sat rides / Wed recovery / Fri strength / Sun rest"
    - "Live Workout launches rs-ride-1 (Intermediate Baseline Ride) and runs the authored interval timeline"
    - "Beginner plan (couch-to-road) still works unchanged (no regression)"
  stuck_tasks: []
  test_all: false
  test_priority: "high_first"
...
agent_communication:
    -agent: "main"
    -comment: "Built the 'Ride Stronger' Intermediate default plan (Phase 1, Weeks 1-4, 12 rides). Source of truth: frontend/src/lib/programs/ride-stronger.ts; rides wired into Live Workout via workout-catalog.ts (RS_WORKOUTS, ids rs-ride-1..12). Backend: ride_stronger_plan.json generated by scripts/export-ride-stronger.js; seeded in server.py; the CTR plan engine (_ctr_state/_ctr_progress/_ctr_plan_response/_ctr_calendar_week) was generalized to drive any structured plan via _struct_ctx('ride-stronger'). Level tags: couch-to-road=Beginner, ride-stronger=Intermediate, build-and-climb level unset (Advanced=coming soon). LEVEL_PLAN Intermediate->ride-stronger. Backend self-tested via curl: plans list OK; recommend(intermediate)->ride-stronger authored:true; assign+GET /plan rich response OK; calendar/week OK. Green Lantern was reset back to couch-to-road after testing. NON-CYCLING days intentionally carry only title+duration (per user). Test creds: greenlantern@roujaune.app / rideon9900 (login path /api/auth/login, field 'token'). To validate ride-stronger UI without disturbing Green Lantern's beginner plan, use profile Change Plan to select Ride Stronger (reset progress), verify Training + Calendar + launching Ride 1, then switch back to From Couch to Road."
