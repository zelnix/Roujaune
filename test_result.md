...
#====================================================================================================
    ...
    -agent: "main"
    -comment: "ROUND 26 — WORKOUT RIDE SCREEN UX fixes (6 items from user). (1) SENSOR PANEL POPUP: BleSensorsPanel now renders inside a RN <Modal transparent animationType='fade' onRequestClose>. Previously its absolute-fill overlay was mounted inside the AppScaffold ScrollView on /connections, so it anchored to scroll content and appeared BELOW the page instead of floating. Modal makes it a true centred popup on connections.tsx AND the workout/virtual-route usages. (2) SPEED TELEMETRY ~40% too slow: backend server.py TrainerSim.step() demo-mode speed formula replaced. Old linear `12 + (power-180)/14 - gradient*0.4` gave ~14 km/h @251W. New realistic aero cube-root model `flat_kmh = 3.6*(power/0.27)**(1/3)` with climb penalty `grade_factor = 1/(1+gradient*0.11)`. Now 250W flat -> 35.1 km/h (was ~17), 251W @7.8% -> 18.9. NOTE native BLE speed (parse.ts speedFromWheel/FTMS 0.01km/h) was already correct — only the demo sim was wrong. (3) TOP TELEMETRY MetricCard secondary text bigger (src/components/workout-live.tsx `m` styles): label 11.5->13 / labelDense 9.5->11.5 (the card TITLE eg SPEED/POWER), pillText 11->12.5 / pillTextDense 9.5->11.5 (the ZONE/ON TARGET pill), sub 12.5->14 (weight 700->800) / subDense 10->12 (the TARGET range line). (4) NOW/NEXT cards bigger stage text (`st` styles): nnName 16->19, nnMeta 12.5->14.5, nnTagNow/NextText 11->12.5, nnCell minHeight 58->68. (5) TERRAIN card bigger (`tc` styles): card minHeight 100->140 + padding/paddingBottom up, title 11->13, statVal 17->23, statLbl 8.5->10.5; elevation graphic `ep.wrap` height 30->42. (6) VIDEO VIEW narrower: app/workout.tsx leftW 184->210 (compact 150->168) & rightW 196->238 (compact 170->194); centerCol is flex:1 so the video auto-shrinks and side cards (Session, Terrain, RouteMap) get more room; phone video height (16:9) recomputes from the measured narrower centerW. MAIN-AGENT VERIFIED: lint clean on both changed components; python speed model sanity (250W->35.1); web app boots to login + home renders (icons OK); backend restarted OK. TEST REQUEST (demo@roujaune.app/demo9900, landscape ~1280x900): FRONTEND — (a) /connections: tap device-btn-trainer -> ble-panel opens as a CENTERED overlay popup (not below the page) and ble-close/backdrop closes it. (b) Start a workout ride (from Today/Workouts pick the scheduled 'Threshold Climb' or any workout -> the immersive workout screen app/workout.tsx). Turn on DEMO (bc-live shows LIVE/DEMO toggle) so telemetry streams. Confirm: metric-speed card shows a realistic km/h (~18-35, NOT ~10-14); the metric card TITLES + TARGET/ZONE secondary lines are clearly larger/readable; the now-next-strip NOW & NEXT stage names are big; terrain-card is taller with bigger numbers; the video 'view' is narrower with wider Session/Terrain columns. (c) In the workout, bc-bluetooth / sensor-health chips open ble-panel as a centered popup too. BACKEND — the telemetry WS demo speed should read realistic (250W~35km/h). Regression: no red screens; prior iters 78-107 PASS. Server ~2026-09."

    -agent: "main"
    -comment: "ROUND 27 — 4 user items. (1) REMOVED the LIVE/DEMO toggle (bc-live pill) from the workout control bar: dropped the button + `live`/`onLive` props from LiveControlBar (src/components/workout-live.tsx) and the two props at the app/workout.tsx call site. settings.demoMode still exists internally (used for the sim connected flag) but is no longer user-visible. (2) SHARE social options (ShareCardModal.tsx, used by milestones/climbs/plan/fitness): added a quick social row above the actions — X (share-x, logo-twitter), WhatsApp (share-whatsapp, logo-whatsapp), Copy (share-copy, expo-clipboard copies a caption). The former primary 'Share' is relabelled 'More…' (share-card-share) and still opens the native OS share sheet with the actual card IMAGE (that sheet is where IG/FB/YouTube live — those platforms don't accept pre-filled posts via links, so branded X/WhatsApp buttons only carry a text caption + tagline). Save unchanged. (3) STRAVA OAuth 'flashed then disappeared' FIX (root cause: Strava rejects custom app schemes as redirect_uri — it only allows the registered HTTPS Authorization Callback Domain). Per integration playbook: backend now has a PUBLIC GET /api/connections/strava/oauth-return (added to auth.py _PUBLIC allowlist) that Strava redirects the browser to, which then bounces to the native deep link roujaune://oauth/strava?code&state via window.location.replace + a visible 'Return to ROUJAUNE' link. authorize endpoint stores both redirect_uri (the HTTPS return, what Strava sees) and app_redirect (the native scheme, for the bounce) in oauth_pending. Frontend ridesync.startConnect special-cases strava: sends Strava redirect_uri = `${EXPO_PUBLIC_BACKEND_URL}/api/connections/strava/oauth-return` while still passing the roujaune:// appRedirect as the WebBrowser returnUrl (so openAuthSessionAsync detects completion), then POSTs code/state to /callback (Strava token exchange needs no redirect_uri). User confirmed their Strava app callback domain = scenic-trainer.preview.emergentagent.com and tests on a real build. (4) SYNC-CARD status: was correct all along — it just showed 'not connected' because #3 never completed; once OAuth completes the account is saved with connection_status='connected' and the card shows Connected + Last synced. MAIN-AGENT VERIFIED via curl: GET /api/connections/strava/oauth-return?code=test123&state=abc -> HTML whose JS/link target is roujaune://oauth/strava?code=test123&state=abc; the ?error=access_denied case forwards the error too. Lint clean on all changed files. TEST REQUEST (demo@roujaune.app/demo9900, landscape): FRONTEND — (1) start a workout ride -> the control bar no longer shows a LIVE/DEMO pill (testID bc-live GONE); other bar buttons (bc-audio, bc-bluetooth, bc-pause, bc-end) still work. (2) Open an achievement share (e.g. Fitness Trends or Plan -> a Share achievement entry) -> ShareCardModal shows a social row with share-x / share-whatsapp / share-copy plus 'More…' (share-card-share) and 'Save' (share-card-save); tapping share-copy shows a 'Caption copied' notice (no crash); share-x/share-whatsapp attempt to open a URL (on web opens a new tab; fine). NOTE Strava full OAuth round-trip (#3) is NATIVE-BUILD ONLY (custom scheme) — cannot be validated in web/Expo Go; backend bounce endpoint is curl-verified above. Prior iters 78-108 PASS. Server ~2026-09."

    -agent: "main"
    -comment: "ROUND 29 — 3 share enhancements (all additive, built on the round 27/28 share pattern). (A) INSTAGRAM FEED: SocialShareRow.tsx now renders TWO Instagram buttons on native — Story (testID share-instagram) + Feed (testID share-instagram-feed). Both save the card image to Photos (expo-media-library/legacy), copy the caption, then Linking.openURL directly in try/catch (Story->instagram-stories://share, Feed->instagram://library, both fall back to instagram://app). Uses openURL (not canOpenURL) so no manifest query-scheme config needed. BOTH IG buttons are hidden on web (Platform.OS check) — on web the row is only X/WhatsApp/Copy. (B) AUTO CAPTION: new CaptionEditor.tsx (YOUR CAPTION label + a one-tap 'Auto stats' pill + multiline input). ShareCardModal (testID share-caption-input, share-caption-auto) builds an auto caption from data.stats pairs; ScenicRecapShareModal (recap-caption-input, recap-caption-auto) builds from journey distance_km/duration_sec/elevation_m. Both replaced their inline caption TextInput with CaptionEditor. (C) SHARE FROM SUMMARY: new RideShareSheet.tsx (bottom-sheet Modal) opened from the end-of-ride summary's Share button (app/summary.tsx BottomActionBar onShare now -> setShowShare(true), no web gate). It reuses CaptionEditor (ride-share-caption-input, ride-share-caption-auto) + SocialShareRow + a 'More…' button (ride-share-more = existing shareRideCard OS sheet) + 'Done' (ride-share-close); sheet backdrop testID ride-share-sheet. initialCaption/autoCaption = existing rideCaption(stats,route); getImageUri = captureRef(cardRef). MAIN-AGENT VERIFIED: web app boots to home (icons OK); lint clean on all 6 touched/new files (only pre-existing 'e'/'radius' unused warnings). TEST REQUEST (demo@roujaune.app/demo9900, landscape ~1280x900), FRONTEND web preview: (1) SCENIC RECAP: go to Virtual Routes -> Saved/Journeys (app/saved-destinations.tsx) -> open a saved journey's Share -> ScenicRecapShareModal: confirm caption input (recap-caption-input) is editable, tapping 'Auto stats' (recap-caption-auto) rewrites it to include distance/time/climb, and the social row shows share-x/share-whatsapp/share-copy (IG buttons share-instagram/-feed should be ABSENT on web); 'More…' (recap-share) + Save present; tapping share-copy shows a 'Caption copied' notice; no red screen. (2) ACHIEVEMENT: from Fitness Trends (app/fitness.tsx) or Training Plan (app/plan.tsx) open a Share achievement -> ShareCardModal: same checks with share-caption-input/share-caption-auto and share-card-share ('More…')/share-card-save. (3) SUMMARY (if reachable by completing/opening a ride summary): tap the Share action in the bottom bar -> RideShareSheet (ride-share-sheet) opens with ride-share-caption-input, ride-share-caption-auto, share-x/-whatsapp/-copy, ride-share-more, ride-share-close. If summary isn't reachable from a fresh session, report that and it will be validated on device. NOTE: Instagram Story/Feed + Save-to-Photos are NATIVE-BUILD ONLY (hidden/no-op on web). Prior iters 78-109 PASS."

    -agent: "main"
    -comment: "ROUND 30 — 'No longer a demo app': fixed the calendar day-tap bug + made placeholder action-toasts functional. (A) CALENDAR DAY TAP / WEEK NAV (root cause: app/calendar.tsx imported useLocalSearchParams but never read `date`, and always hard-coded selected=Tuesday; the backend /calendar/week ignored any target date for structured/demo plans). BACKEND routes/plan.py get_calendar_week now takes optional `date` (alias, focus_date) — resolves the structured weeks_map/ride/supp state, then via new _week_containing() returns the plan week whose 7-day span contains the date; if the date is outside the plan (or a non-demo week for build-and-climb) it returns _free_calendar_week(date). Default (no date) behaviour unchanged (current week / demo fixed week / free week). FRONTEND src/lib/calendar.ts useCalendarWeek(focusDate?, start) sends ?date= when focusDate set. app/calendar.tsx reads params.date -> focusDate state (kept in sync on nav), passes it to the hook, and an effect focuses the matching day index after each week load (falls back to week.selected_date then Tuesday). DateControls prev/next now setFocusDate(shiftISO(week.start_date, ±7)); Today -> setFocusDate(todayISO()); Settings -> router.push('/settings'); the phantom 'Week' dropdown is now a static label (src/components/calendar.tsx). (B) CALENDAR ADD/QUICK-ACTIONS: empty TRAINING/FB50/RECOVERY/SCHEDULED slots + QuickActionsCard now route to /workout-list?type=&date=<day> (goSchedule) so the picked session is scheduled onto that exact date; workout-list.tsx addToCalendar now passes params.date to scheduleWorkout and toasts 'Added X to <day>'; 'Reschedule' quick-action opens the swap sheet (or scheduling when no custom plan). (C) PLAN SCREEN: KeyWorkoutsCard onView/onNext -> /workout-list?type=all (viewAllWorkouts); WeeklyLoadCard 'This Plan' filter made a static label (no phantom dropdown). (D) SUMMARY: SyncExportRow items (Strava/Apple Health) now open the real connect flow (/connections) instead of a status toast. (E) WORKOUTS builder now REAL: new backend POST /api/catalog (routes/catalog.py) creates a rider custom workout (id custom-*, custom:true) in rider_workouts (returned by resolve_catalog); new src/lib/custom-workouts.ts (createCustomWorkout w/ TSS/IF/zone estimator, listCustomWorkouts, deleteCustomWorkout); new app/workout-builder.tsx (name/desc + segment editor: label, zone picker, minutes & %FTP steppers, reorder/remove, live duration/TSS/IF/effort, Save -> POST then open the new workout; ZWO import via expo-document-picker + minimal parser). app/workouts.tsx: 'My Workouts' tab lists custom workouts (tap opens, delete), Create/Builder quick-actions -> /workout-builder, Import -> /workout-builder?import=1, 'Compare Types' opens a real CompareTypesModal. EXCLUDED per user: Home 'Joining a group ride' toast + side-nav 'coming soon' items (deferred). MAIN-AGENT VERIFIED via curl (demo@roujaune.app): /calendar/week default -> 2025-05-12 demo week; ?date=2025-05-14 -> same demo week; ?date=2025-06-02 -> free week (free:true) 2025-06-02..08. POST /api/catalog created custom-* (custom:true), GET /api/catalog lists it, DELETE .../reset removes it. Lint clean on all changed FE files; web app boots to login. TEST REQUEST (demo@roujaune.app/demo9900, landscape ~1280x900): FRONTEND — (1) /plan: tap a day in the mini-calendar and a 'This Week's Plan' row -> navigates to /calendar and that day is focused/highlighted (NOT a toast). (2) /calendar: prev/next arrows change the week (range label updates); Today jumps back; tapping a day-head selects it; an empty TRAINING/RECOVERY/FB50 slot opens /workout-list pre-filtered with a date; picking 'Add to calendar' schedules onto that day and it appears under 'Scheduled by you' after returning. (3) /plan Key Workouts 'View All'/next arrow -> /workout-list (type=all). (4) /workouts: 'Compare Types' opens the compare modal; My Workouts tab -> 'Create Custom Workout' opens /workout-builder; build a session (add/remove segments, change zone/min/%), Save -> lands on the new workout in /workout-list and it shows under My Workouts; delete it. Import quick-action opens the builder (document picker headless may cancel — expected). (5) summary Sync & Export Strava/Apple tiles route to /connections. BACKEND — /api/calendar/week?date=<in-plan> vs <out-of-plan>; POST/GET/DELETE /api/catalog custom workout lifecycle. Prior iters 78-109 PASS. Server ~2026-06."


    -agent: "main"
    -comment: "ROUND 33 — Coach-chat 'change my training plan schedule' had no effect (user report: coach confirmed but nothing moved). Fixed 3 root causes. (1) INTENT: _START_RE (routes/coach.py) required literal 'start (my|the) plan' so 'start my TRAINING plan tomorrow' never matched -> broadened to (start|begin|kick off|push|move|shift|change|adjust|set) + optional words + (plan|schedule|training), plus reset ... (plan|start|schedule|training). (2) NO RE-ANCHOR: _reset_plan_start (routes/plan.py) only wrote plan_state.start_date, but plan+calendar dates derive from each week's start_date in the plan DEFINITION. Added _rebase_plan_def() to re-date every week in the rider's snapshot (training_plans.definition): week1=new_start, +7d per week, persisted; sets duration_weeks_override into the def when compressing to a hard event. Also get_calendar_week now uses _struct_ctx_for_rider for couch-to-road/ride-stronger/ride-beyond (was global plan_engine.CTR_WEEKS) so the calendar reads the same rebased snapshot as get_plan. (3) NO UI REFRESH: the Today screen's CoachChatModal didn't refresh after a change. Added cross-component signal in src/lib/plan.ts (subscribePlanChange/notifyPlanChanged); usePlan() and useCalendarWeek() subscribe and re-fetch; CoachChatModal calls notifyPlanChanged() on res.plan_updated (in addition to the existing onPlanUpdated used only by plan.tsx). MAIN-AGENT VERIFIED — curl (greenlantern@roujaune.app/rideon9900, couch-to-road): POST /api/plan/start-date and POST /api/coach/chat 'start my training plan tomorrow' both move start_date 2026-12-01 -> 2026-09-09, get_plan next workout date_label -> 'Wed 9 Sep', and /api/calendar/week?date=<tomorrow> returns the plan week with the shifted sessions (Welcome Ride/Pedal Smoothly/First Endurance Ride). BROWSER E2E (greenlantern, 1280x900): opened Message Alberto on Today, sent the message -> green 'Plan updated · Restarted From Couch to Road on 2026-09-09' banner + in-persona confirmation; after closing the chat the Today card's NEXT SCHEDULED WORKOUT updated live from 'Tue 1 Dec' to 'Wed 9 Sep' with NO reload. Lint clean. NOTE: day_name labels on the calendar remain the plan-template slot labels (MON..SUN) even when the new start lands mid-week — cosmetic, dates are correct. Prior iters 78-112 PASS. Server ~2026-06."


    -agent: "main"
    -comment: "ROUND 34 — built the 4 follow-ups. (A) WEEKDAY LABELS: _ctr_calendar_week (services/plan_engine.py) now derives day_name from the real date (MON..SUN by dt.weekday()) instead of the plan-template slot label, so a mid-week plan start shows correct weekdays. Verified: /calendar/week?date=2026-10-14 -> day0 WED, sequential THU..TUE. (B) MISSED WORKOUT PROMPT: MissedWorkoutBanner.tsx rebuilt into an interactive 'skip or reschedule?' card — Skip and 'Reschedule to <suggested day>' buttons call POST /api/rider/missed/resolve (home-notices.ts resolveMissed) then notifyPlanChanged + refresh; shows a brief done state then clears when count hits 0. home-notices.ts exposes id/suggested_date + a refresh(). Verified in-browser (greenlantern) after seeding a past-due scheduled_workout: card rendered 'Missed ride — Endurance Spin' with Skip + 'Reschedule to Wed, Sep 9'; tapping Reschedule resolved and removed the banner. (C) UNDO RESCHEDULE: backend snapshots the full pre-change rider definition + plan_state into plan_undo inside _reset_plan_start (deepcopy), and POST /api/plan/undo-reschedule restores it verbatim and deletes the snapshot; _reset_plan_start returns can_undo:true and coach/chat returns can_undo. Frontend: src/lib/plan.ts adds resetPlanStart()/undoReschedule() (both call notifyPlanChanged); CoachChatModal shows an 'Undo' button in the plan-updated notice; Plan-screen change shows an Undo toast. Verified via curl: reset 2026-10-05 -> 2026-11-16 -> undo -> 2026-10-05. (D) PLAN RESET BUTTON: new 'Change start date' button in the Plan header (hidden when no_plan) opens ChangeStartDateModal.tsx — a web+native-safe picker (day stepper + Tomorrow/Next Monday/In 2 weeks chips + past-date guard) -> resetPlanStart(date) -> refresh + Undo toast. Verified in-browser: modal opened, 'In 2 weeks' -> Tue 22 Sep, confirm re-anchored the plan live (calendar + This Week's Plan updated) and showed 'Restarted From Couch to Road on 2026-09-22 … Undo' toast; no crash. Lint clean on all changed FE files. Prior iters 78-112 + rounds 32/33 PASS. Server ~2026-06."


    -agent: "main"
    -comment: "ROUND 35 — 'Coach could not move plan to start this Monday'. Root cause: on the DEMO plan (build-and-climb) the coach chat replied 'Restarted Build & Climb …' (plan_updated:true) but nothing moved — build-and-climb is a static showcase plan with NO dated weeks def, so _reset_plan_start's rebase was gated to structured/custom plans only and skipped it; its calendar came from a hardcoded 2025-05-12 demo week ignoring plan_state.start_date. (Real structured plans couch-to-road/ride-stronger/ride-beyond/custom already worked — confirmed via curl.) FIX (routes/plan.py): (1) new _shift_demo_week(anchor) re-dates the demo week so day0=anchor (weekday labels recomputed); new _plan_start_override(plan_id) reads plan_state.start_date. (2) get_calendar_week build-and-climb branch now: if an override start exists, returns the demo week shifted to it (dates within its 7-day span) else a free week; with no override keeps the original fixed 2025-05-12 showcase. (3) _reset_plan_start now ALWAYS snapshots pre-change plan_state (+def when structured) into plan_undo and rebases the def only when the plan has one — so build-and-climb records undo + moves via plan_state. (4) get_plan overlays plan_start_override into the non-structured/demo plan response so the header reflects the new start. Also broadened _START_RE (routes/coach.py) to also fire on 'start/begin/kick off + this/next/on + <weekday|today|tomorrow>' (e.g. 'start this Monday', 'starting tomorrow') WITHOUT hijacking missed-workout 'reschedule … to Monday' or analytics 'how did I do on Monday?'. MAIN-AGENT VERIFIED — curl (demo@roujaune.app): chat 'Move my training plan to start this Monday' -> plan.start_date 2026-09-14, /calendar/week?date=2026-09-14 returns the demo week (free:None) re-dated MON Endurance Ride / TUE Threshold Climb / WED Recovery Ride; undo restores. Clean A->B->undo (greenlantern) = A. Regex unit tests: 7/7 incl. negatives. BROWSER (demo): coach chat shows green 'Plan updated · Restarted Build & Climb on 2026-09-14' + Undo and in-persona confirmation. Demo account restored pristine (start_date None -> default 2025-05-12 week). Lint clean. Prior iters 78-112 + rounds 32-34 PASS. Server ~2026-06."
