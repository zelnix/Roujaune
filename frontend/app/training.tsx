...
import { resolveWorkout } from "@/src/lib/workout-catalog";
...
export default function TodaysTraining() {
  ...
  const startRide = () => router.push(activeId ? ({
    pathname: "/workout",
    params: {
      workoutId: activeId,
      title: activeWorkout?.name ?? meta?.title,
  ...
  const onSelectNav = (key: string) => {
    ...
    const routes: Record<string, string> = { calendar: "/calendar", routes: "/virtual-route", progress: "/progress", community: "/community", connections: "/connections", settings: "/settings", help: "/help" };
    if (routes[key]) {
