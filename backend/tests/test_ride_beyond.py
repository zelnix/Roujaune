"""Iter 30 — Ride Beyond (Advanced) Phase 1 authored plan (4 weeks / 12 rides)
+ Advanced onboarding recommendation + calendar wiring + regression on
Ride Stronger and Couch-to-Road.
...
# --------------------- 1. GET /api/plans lists ride-beyond ---------------------
class TestPlansCatalog:
    def test_ride_beyond_is_advanced_structured_4_weeks(self, api):
        ...
        wk = rb.get("week_count") or rb.get("duration_weeks")
        assert wk == 4, f"expected week_count/duration_weeks==4, got {wk} — full={rb}"
        # Regression: other authored plans still listed
        assert "couch-to-road" in by_id
        assert "ride-stronger" in by_id
    ...
    def test_advanced_profile_recommends_ride_beyond(self, api):
        r = api.post(
            ...
            json={
                "experience_years": 12,
                "weekly_hours": 12,
                "longest_ride_min": 240,
                "can_ride_60min": True,
                "structured_before": True,
            },
        )
        assert r.status_code == 200, r.text
        data = r.json()
...
class TestAssignRideBeyond:
    def test_assign_and_fetch_structured_plan(self, api):
        ...
        assert weeks_label.replace(" ", "") == "1/4", f"progress.weeks: {weeks_label!r}"

        # Phases include 'Performance Foundation'
        phases = data.get("phases") or []
        names = [(p.get("name") or "").lower() for p in phases]
        assert any("performance foundation" in n for n in names), f"phases: {phases}"
...
class TestCalendarWeek:
    def test_calendar_week_maps_days_to_rides(self, api):
        ...
        # Wed strength (FB50), Fri recovery, Mon/Sun rest/recovery
        ...
        assert is_rest(by_day["MON"]) or is_recovery(by_day["MON"]), f"MON not rest/recovery: {by_day['MON']}"
        assert not has_cycling(by_day["SUN"]), f"SUN should not be a cycling ride: {by_day['SUN']}"


# --------------------- 5. Regression: Ride Stronger still intact ---------------------
class TestRideStrongerRegression:
    def test_ride_stronger_still_12_weeks(self, api):
        r = api.post(
            f"{BASE_URL}/api/rider/plan",
...
# --------------------- 6. Regression: Couch-to-Road (also LEAVES USER on CTR) ---------------------
