...
def admin_api():
    ...
    plans_router dependency. Not exposed to riders (the mobile app never
    calls it — plan switching goes through /api/onboarding/* instead)."""
    s = requests.Session()
    s.headers.update({"Content-Type": "application/json"})
...
class TestPlansCatalog:
    def test_ride_beyond_is_advanced_structured_12_weeks(self, admin_api):
