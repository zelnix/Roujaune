...
- **Roadmap:** Phase 3 (DONE) = PMC + PRs + compare. Outdoor rides come in via (a) **file upload** (.fit/.gpx/.tcx — covers every Garmin/Wahoo/Hammerhead bike computer) and (b) **Garmin Connect** cloud sync (existing `providers/garmin.py` in the generic `routes/connections.py` pipeline). Strava provider was removed per product decision. Native map (react-native-maps) later; SVG route used everywhere for now.
