...
- **Roadmap:** Phase 3 (DONE) = PMC + PRs + compare. Phase 4 (DONE, keys pending) = Strava sync — `providers/strava.py` (OAuth authorization-code, pulls rides + streams → route_data samples/power_curve) registered in the generic `routes/connections.py` pipeline; shows in the Connections screen as "Setup required" until `STRAVA_CLIENT_ID`/`STRAVA_CLIENT_SECRET` env are set (custom-scheme OAuth needs a native/published build). Garmin/Google Fit/Apple Health adapters already exist. Native map (react-native-maps) later; SVG route used everywhere for now.
